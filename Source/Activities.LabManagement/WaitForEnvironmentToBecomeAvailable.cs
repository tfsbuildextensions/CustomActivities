 ...
 using System.Threading;
  ...
  public class WaitForEnvironmentToBecomeAvailable : CodeActivity
  {
    ...
    protected override void Execute(CodeActivityContext context)
    {
      ...
      int maximumWaitTime = context.GetValue(MaximumWaitTimeSeconds);
      ...
      while (maximumWaitTime == 0 || (DateTime.Now < endTime))
      {
        ...
        foreach (string name in environmentNames)
        {
          ...
          if (!File.Exists(Path.Combine(lockingUNCShare, name)))
          {
            ...
            foreach (LabEnvironment oEnvironment in lstEnvironments)
            {
              if (oEnvironment.Name.Equals(name, StringComparison.OrdinalIgnoreCase))
              {
                //-- Get the environment In-Use information...
                LabEnvironmentInUseMarker marker = oEnvironment.GetInUseMarker();
                if (marker == null)
                {
