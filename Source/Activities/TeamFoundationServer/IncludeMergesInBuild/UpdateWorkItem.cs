...
namespace TfsBuildExtensions.Activities.TeamFoundationServer.IncludeMergesInBuild
{
	...
	public sealed class UpdateWorkItem : CodeActivity<bool>
	{
		...
		protected override bool Execute(CodeActivityContext context)
		{
			...
			var typeDictionary = WorkItemTypeFieldCache.Get(context);
			...
			if (hasIntegrationBuildField)
			{
				...
				workItem[IntegrationBuildFieldRef] = BuildNumber.Get(context);
				workItem.History = "The Fixed In field was updated as part of associating work items with the build.";
				if (!workItem.IsValid())
				{
					WarningMessage.Set(context, CombineInvalidFields(workItem));
					return false;
				}
				return true;
			}
			WarningMessage.Set(context, string.Format("TF42093: The work item {0} could not be updated with build information. The field {1} is not available on this work item.",
				workItem.Id, IntegrationBuildFieldRef));
			return false;
		}
		...
		private static string CombineInvalidFields(WorkItem workItem)
		{
			...
			return string.Format("TF42097: A work item could not be saved due to a field error. The following fields have incorrect values:{0}", builder.ToString());
		}
	}
}
