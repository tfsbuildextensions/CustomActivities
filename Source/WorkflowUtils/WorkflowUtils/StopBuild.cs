...
namespace WorkflowUtils
{
    ...
    public sealed class StopBuild : CodeActivity<IQueuedBuild>
    {
        ...
        {
            IQueuedBuild buildToRetrieve = bs.GetQueuedBuild(buildId, QueryOptions.None);

            if (buildToRetrieve.Status == QueueStatus.Postponed || buildToRetrieve.Status == QueueStatus.Queued)
                buildToRetrieve.Cancel();
            else if (buildToRetrieve.Status == QueueStatus.InProgress)
